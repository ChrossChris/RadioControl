...
{
  const Joystick* thrust   = RcHardware::getJoystick(JoystickType::THRUST);
  const Joystick* rudder   = RcHardware::getJoystick(JoystickType::RUDDER);
  const Joystick* elevator = RcHardware::getJoystick(JoystickType::ELEVATOR);
  const Joystick* aileron  = RcHardware::getJoystick(JoystickType::AILERON);

  if ((thrust == nullptr) || (rudder == nullptr) ||
      (elevator == nullptr) || (aileron == nullptr)) return;
