...
void readJoysticks()
{
  ...
  for (int i = 0; i < CHANNELS; i++)   
  {
    ...
    if    (i != MOTOR) joysticks[i] = constrain(joysticks[i] - centerPos[i],  joystickLimits[i][MIN_POTI], joystickLimits[i][MAX_POTI]);
    else
    {
      joysticks[MOTOR] = map(joysticks[MOTOR], centerPos[MOTOR], joystickLimits[MOTOR][MAX_POTI], joystickLimits[MOTOR][MIN_POTI],  joystickLimits[MOTOR][MAX_POTI]);
      joysticks[MOTOR] = constrain(joysticks[MOTOR], joystickLimits[MOTOR][MIN_POTI], joystickLimits[MOTOR][MAX_POTI]);
    }
    ...
    // -  0.. 10% Geberstellung gehen auf       0% maximaler Gasstellung
    // - 10.. 20% Geberstellung gehen auf  0.. 30% maximaler Gasstellung
    // - 20.. 90% Geberstellung gehen auf 30.. 70% maximaler Gasstellung
    ...
    else if (i == MOTOR)
    {
      ...
      const int x1 = (1*joystickLimits[i][MAX_POTI])/10; //   10%
      const int x2 = (2*joystickLimits[i][MAX_POTI])/10;  //  20%
