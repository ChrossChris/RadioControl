...
#include "C:\Development\Elektronik\Fernsteuerung_FM6014\git-repository\inc\multiplex_heron.h"
#include "C:\Development\Elektronik\Fernsteuerung_FM6014\git-repository\inc\dpower_streamline.h"
#include "C:\Development\Elektronik\Fernsteuerung_FM6014\git-repository\inc\graupner_eps2000.h"
#include <Bounce2.h>
#include <EEPROM.h>
...
void setup()
{
  ...
  // joystickLimits[MOTOR][MAX_POTI] = POTI_MOTOR_MAX;
    ...
    // maximale Joystickausschläge: Höhenruder
  joystickLimits[HOEHE][MIN_POTI] = POTI_HOEHE_MIN;
  joystickLimits[HOEHE][MAX_POTI] = POTI_HOEHE_MAX;
  ...
  // Nullpunkt mitteln bei Starten der Fernsteuerung
  const int amount = 10;
  for (int i = 0; i < CHANNELS; i++) centerPos[i] = 0;
  ...
  centerPos[HOEHE] /= amount;
  centerPos[SEITE] /= amount;
  centerPos[QUER]  /= amount;
  centerPos[MOTOR] /= amount;
...
void loop()
{
  ...
  switch (state)
  {
    ...
    case __RUNNING__:
      if      (    button.rose()             )      state = __RUNNING__;
               ...
               && ((millis()-startTime) < 120000))  state = __TRIMMING__;
      else if (   (button.read()     == HIGH)  
               && (button.duration() >  1500))      joysticks[MOTOR] = 1023;
  ...
  else if (state == __RUNNING__)
  {
    ...
    {
      trimmHoehe = 1023 - analogRead(POTI_LINKS_1);  // Wechsel der Drehrichtung für Trimmung
    ...
    // if (switchLinksUnten == OFF)  // Schalter funktioniert leider nicht mehr zuverlässig, daher wird er mit dem anderen gekoppelt
    if (switchLinksOben == OFF)         
    {
